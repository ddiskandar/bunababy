<div class="px-4 py-2 border rounded border-brand-50" x-data="{ expanded: false }">
    <div class="flex items-center justify-between ">
        <div>
            <span class="ml-2 font-semibold">{{ $room->name }}</span>
        </div>
        <button
            ...
            <div class="ml-2">
                <svg xmlns="http://www.w3.org/2000/svg" :class=" expanded == true ? 'rotate-180' : ''"
...
{{--
<div>
    <div class="mb-4 font-semibold lg:text-lg">Pilih Jadwal Tersedia di {{ $place->name }}</div>

    <div class="p-4 border rounded border-brand-50" >
            ...
            <div class="grid grid-cols-7 gap-6">
                ...
                @foreach ($data as $day)
                    @if (\Carbon\Carbon::parse($day['date'])->gte(today()))
                        ...
                        @elseif($day['status'] == 'tersedia')
                            <div class="flex flex-col items-center p-4 rounded cursor-pointer hover:bg-brand-50">
                                <span>{{ \Carbon\Carbon::parse($day['date'])->isoFormat('DD') }}</span>
                                <div class="w-3 h-3 bg-blue-400 border-2 border-white rounded-full"></div>
                            </div>

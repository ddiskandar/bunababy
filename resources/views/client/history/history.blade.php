<div class="relative">
    ...
    <div class="max-w-screen-sm min-h-screen mx-auto my-0">
        <div class="flex flex-wrap justify-center gap-2 py-4 ">
            <button wire:click="$set('filterStatus', '')" @class([
            ...
            <button wire:click="$set('filterStatus', '2')" @class([
                'py-1 text-xs font-semibold px-4 border hover:bg-brand-200 hover:text-white transition-all border-brand-200 rounded-full',
                'bg-brand-200 text-white' =>
                    (int) $filterStatus === \App\Models\Order::STATUS_LOCKED,
                'text-brand-200 bg-white' =>
                    (int) $filterStatus !== \App\Models\Order::STATUS_LOCKED,
            ])>
                Aktif
            </button>
            <button wire:click="$set('filterStatus', '3')" @class([
                'py-1 text-xs font-semibold px-4 border hover:bg-brand-200 hover:text-white transition-all border-brand-200 rounded-full',
                'bg-brand-200 text-white' =>
                    (int) $filterStatus === \App\Models\Order::STATUS_FINISHED,
                'text-brand-200 bg-white' =>
                    (int) $filterStatus !== \App\Models\Order::STATUS_FINISHED,
            ])>
                Selesai
            </button>
            <button wire:click="$set('filterStatus', '1')" @class([
                'py-1 text-xs font-semibold px-4 border hover:bg-brand-200 hover:text-white transition-all border-brand-200 rounded-full',
                'bg-brand-200 text-white' =>
                    (int) $filterStatus === \App\Models\Order::STATUS_UNPAID,
                'text-brand-200 bg-white' =>
                    (int) $filterStatus !== \App\Models\Order::STATUS_UNPAID,
            ])>
                Pending
        ...
        <div class="fixed inset-x-0 bottom-24 z-90">
            <div class="flex items-center justify-center">
                <a href="{{ route('order.create') }}"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 text-white" width="24" height="24"

        ...
        <div class="w-full p-3 border-b border-gray-100 grow">
            <div class="relative">
                ...
                <input wire:model="filterSearch"
                    ...
                    type="search" placeholder="Mencari berdasarkan nama, wilayah, nomor WA atau IG ..." />
            </div>
        </div>
        <!-- END Card Header -->
                        ...
                        @forelse ($clients as $client)
                            <tr @class([
                                ...
                                <td class="p-3 pl-6 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <img src="{{ $client->profile_photo_url }}" alt="User Avatar"
                                ...
                                <td class="p-3 whitespace-nowrap">
                                    ...
                                    @if (isset($client->profile->ig))
                                        <p class="text-slate-600">
                                            <a href="https://www.instagram.com/{{ $client->profile->ig }}"
                                                target="_blank">instagram.com/{{ $client->profile->ig }}</a>
                                        </p>
                                    @else
                                        <span> - </span>
                                    @endif
                                ...
                                <td class="p-3 whitespace-nowrap">
                                    {{ $client->latestReservation ? $client->latestReservation->startDateTime->diffForHumans() : 'Belum pernah reservasi' }}
                                </td>
                                <td class="p-3 whitespace-nowrap">
                                    <div class="flex flex-wrap gap-2 whitespace-nowrap">
                                        @foreach ($client->tags as $tag)
                                            <div
                                                class="inline-flex items-center px-4 py-1 space-x-1 text-xs font-semibold leading-4 border rounded-full text-slate-600 bg-slate-50 border-slate-200">
                                                {{ $tag->name }}
                                            </div>
                                        @endforeach
                                    </div>

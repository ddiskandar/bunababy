<!-- Card -->
<div class="flex flex-col mt-8 overflow-hidden bg-white rounded shadow-sm" wire:ignore wire:init="loadData"
    x-data="{
        data: @entangle('data'),
        ...
        init() {
            ...
            Livewire.on('this-month-orders-updated', () => {
                reservationThisMonthChart.data.labels = this.data.labels;
                reservationThisMonthChart.data.datasets = this.data.datasets;
    ...
    <div class="w-full px-6 py-3 bg-gray-50 sm:flex sm:justify-between sm:items-center">
        ...
        <div class="flex items-center gap-4 mt-3 text-sm text-center sm:mt-0 sm:text-right">
            <input wire:model="selectedMonth" type="month"
    ...
    <div class="relative w-full p-5 lg:p-6">
        <div wire:loading.class="opacity-100" wire:loading.class.remove="opacity-0"
            class="absolute flex items-center justify-center w-full transition opacity-0 h-96">
        ...
        <canvas wire:loading.class="opacity-0" class="relative w-full transition " x-ref="reservationThisMonth"
            height="280"></canvas>
    </div>
    <!-- Card Body -->
</div>

        ...
        <div class="w-full p-3 border-b border-gray-100 grow">
            <div class="relative">
                ...
                <input wire:model="filterSearch"
                    ...
                    type="search" placeholder="Mencari berdasarkan nama client, order atau besar pembayaran ..." />
            </div>
        </div>
        <!-- END Card Header -->
                            ...
                            <tr @class([
                                ...
                                <td class="p-3 align-top ">
                                    ...
                                    @if ($payment->order->getRemainingPayment() > 0)
                                        <p class="text-red-600">{{ rupiah($payment->order->getRemainingPayment()) }}
                                        </p>
                                    @else
                                        <div class="flex items-center">
                                ...
                                <td class="p-3 align-top ">
                                    <p class="font-semibold truncate w-52 text-slate-800">
                                        {{ $payment->order->client->name }}</p>
                                    ...
                                    <div class="flex justify-center space-x-2">
                                        @if (
                                            $payment->status() === 'Waiting' ||
                                                auth()->user()->isOwner())
                                            <button wire:click="showEditPaymentDialog({{ $payment->id }})"
        ...
        <form wire:submit.prevent="save">
            ...
            <div class="h-64 px-1 mt-2 space-y-3 overflow-y-auto">
                <div class="space-y-1">
                    ...
                    <x-input wire:model.lazy="state.value" x-mask:dynamic="$money($input, ',')" class="w-full"
                        type="text" id="state.value" />
